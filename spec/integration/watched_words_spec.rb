...
describe WatchedWord do
  ...
  context "flag" do
    ...
    it "should flag the post as inappropriate" do
      should_flag_post(tl2_user, "I thought the movie was #{flag_word.word} bad.", Fabricate(:topic, user: tl2_user))
    end

    it "should look at the title too" do
      should_flag_post(tl2_user, "I thought the movie was not bad actually.", Fabricate(:topic, user: tl2_user, title: "Read my shocking #{flag_word.word} review!"))
    end

    it "shouldn't flag posts by admin" do
      should_not_flag_post(admin, "I thought the movie was #{flag_word.word} bad.", Fabricate(:topic, user: admin))
    end

    it "shouldn't flag posts by moderator" do
      should_not_flag_post(moderator, "I thought the movie was #{flag_word.word} bad.", Fabricate(:topic, user: moderator))
    end
    ...
    it "is compatible with flag_sockpuppets" do
      ...
      sockpuppet_post = create_post(user: user2, topic: first.topic, raw: "I thought the movie was #{flag_word.word} bad.")
